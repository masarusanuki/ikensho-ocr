...
def build(out_path):
    ...
    # 6.6 新しい正解データ（seigo2・1,000通）
    s = blank(prs)
    heading(s, "ベンチマーク②c 新しい正解データ1,000通（Word入力と手書きが混在）",
            "読みやすさの4段階から16通ずつ・64通で実測。□ひとつを1件として数えた")
    table(s, [
        ["読みやすさ", "チェック欄", "文字欄", "件数（1,000通中）"],
        ["活字", "86.5 %", "84.2 %", "350"],
        ["標準", "81.6 %", "84.2 %", "204"],
        ["達筆（連綿・寝せ字）", "78.1 %", "39.7 %", "227"],
        ["汚い（つぶれ・重なり）", "66.4 %", "16.1 %", "219"],
    ], y=1.95, size=14, widths=[3.4, 2.4, 2.4, 3.2],
       highlight=lambda r, c, v: r == 4)
    bullets(s, [
        ("崩れているのは「汚い」の1種類に集中している",
         "文字は 16.1%。実質読めていない。取り込みの品質がそのまま精度になる"),
        ("枠単位で数えると 89.36%（4,895枠）。seigo の 99.08% とは10ポイント差",
         "数え方を揃えた上での差なので、本物の差。しきい値は seigo に合わせたままで未調整"),
        ("テンプレートの座標の取り違えで、中央寄せの欄がそろって壊れていた",
         "年齢12.5%→54.2% / 身長6.3%→48.1% / 体重15.6%→58.3%。目視では気づけなかった"),
    ], y=4.4, size=15)
    note(s, "数字は「どの正解データで、何を1件と数えたか」と必ず一緒に見てください。", y=6.5)
    ...
    table(s, [
        ["懸念", "中身", "いま言えること"],
        ["読み取りは完全ではない", "チェック 99.08%（seigo・18,600枠）／"
         "89.36%（seigo2・4,895枠）", "必ず原本と照らして確認する運用が前提"],
        ["つぶれ・重なりの多い紙", "チェック66.4% / 文字16.1%。1,000通中219通",
         "取り込みの品質がそのまま精度になる"],
        ...
        ["解像度の低い入力", "75dpi 相当でも読めるが、年の精度は落ちる",
         "200dpi以上での取り込みを勧める"],
        ["正解データが2種類とも合成", "seigo 100通・seigo2 1,000通。どちらも生成したもの",
         "実データでの検証が要る"],
