...
## 5.2 新しい正解データ（seigo2）で測る

1,000通ぶん。Word入力350通・読みやすさ4段階・チェックの表現15通り・
スキャン風600通が入っているので、**条件ごとに切り分けて**見ます。

```bash
.venv/bin/python tools/benchmark_seigo2.py --jobs 6 --limit 64 --by 読みやすさ
.venv/bin/python tools/benchmark_seigo2.py --jobs 6 --limit 64 --by mode
.venv/bin/python tools/benchmark_seigo2.py --jobs 6 --limit 64 --by チェック表現
```

- **`python3` ではなく `.venv/bin/python`** を使ってください
  （システムの python には pypdfium2 が入っていません）
- `--limit` は**軸ごとに同じ数ずつ**抜きます。単に間引くと偏ります
  （1,000通を25通おきに取ったら40通すべて手書きになりました）
- 1通あたり1分前後かかります。時間のほとんどは**ラベルの読み取り**で、
  チェック欄の判定そのものは0.3秒です

出力は3つの数字が並びます。**それぞれ数え方が違うので混ぜないでください。**

| 行 | 何を1件と数えるか |
|---|---|
| 枠単位 | □ひとつ。印あり／なしが合っているかの0か1。**`seigo/` の 99.08% と同じ数え方** |
| チェック | 質問ひとつ。答えの言葉の文字列の近さ（0〜1） |
| 文字 | 文字欄ひとつ。同上 |

正解データは質問ごとの答えの言葉しか持たないので、枠単位の正解は
`expected_opts()` が展開しています（1,000通で 98.9% を展開できます。
残りの「その他(自由記述)」は**数えずに除きます**）。

`seigo2/` もリポジトリに入れていません。

---

## 5.3 誤字表を作り直す

OCR の読み崩れは、**規則で直せるものだけ**を自動で直します（LLM は使いません）。
誤字表は手で書かず、種から機械で作ります。

```bash
python3 tools/fetch_municipalities.py     # 総務省から市区町村を取る（1,914件）
python3 tools/build_word_fixes.py         # 誤字表を作り直す（dict/word_fixes.json）
...
| 種 | 中身 |
|---|---|
| `dict/care_terms.json` | 自由記述に出る介護・医療の語（手で書いた378語） |
| `dict/ocr_confusions.json` | 形の似た字の組（漢字162・かな43） |
| `dict/*.json` | 傷病名・部位・診療科・感染症・市区町村（合わせて2,524語） |
| `dict/mined_fixes.json` | 実測から拾った語（129語） |

**種の語に検証データから採ったものを入れないこと。**
`seigo2` の生成元には正解の文がそのまま入っているので、そこから語を採ると
「答えを写して測る」ことになり、数字が意味を失います。

### 実測から語を拾う

```bash
# 1. 読み取り結果と正解の対を1回だけ落とす（88通で約90分）
.venv/bin/python tools/mine_word_fixes.py dump --limit 90 --jobs 6

# 2. 以後はこのファイルだけで、拾い方を何度でも試せる（1秒）
.venv/bin/python tools/mine_word_fixes.py mine --train 0.66 --min-count 2
```

**拾う通と測る通を必ず分けます。** 同じ通で拾って同じ通で測れば必ず良くなりますが、
それは何も言っていないのと同じです。いまの基準値は
**59.46% → 59.80%（良化28件・悪化1件）**（58通で拾い、30通で測定）。

拾い方の要点:

- **直した側が本物の語になるように切り出す。** 文脈を機械的に2文字足すと
  `る観繁を行 → る観察を行` となり、その文でしか効きません。
  語に合わせて切ると `観繁 → 観察` になり、別の文でも効きます
- **本物の語は弾く。** 正解文に出てこないことだけを条件にすると、
  `京都市左京区 → 京都市右京区` のような表を作ってしまいます（左京区は実在）。
  辞書に載っている語は、語の一部として含む場合も弾きます
...
## 9.5 源内（デジタル庁の生成AI基盤）向けの受け口

[源内](https://github.com/digital-go-jp/genai-web)は外部の REST API を
「行政実務用AIアプリ」として登録して呼び出せます。その取り決めに合わせた入り口が
`python/ikensho_ocr/gennai.py` と `server.py` にあります。

```
POST /api/gennai        1通を読んで返す
GET  /api/gennai/form   源内の登録画面に貼る「リクエスト形式」JSON
```

**既定では閉じています（404）。** 開くのは `ikensho serve --gennai` のときだけです。
主治医意見書は要配慮個人情報で、源内はクラウド上のサービスなので、
「すべて端末内で処理する」という本アプリの原則から外れるためです。

### 9.5.1 取り決め

| | |
|---|---|
| 受け取り | `{"inputs": {...}}`。ファイルは `inputs.files` に base64 |
| 返し | `{"outputs": "Markdown のテキスト"}` |

**受け取りはゆるく作ってあります。** 仕様書の中で書き方が2通りあるためです。
...
どちらも受け、`data:...;base64,` 付きも受けます。
拡張子が無い・当てにならない場合は中身（`%PDF`・PNG署名など）から判定して付けます。
**`imaging.load_any` は拡張子で処理を振り分けるので、付けないと読めません。**

### 9.5.2 気をつけたところ

| | なぜ |
|---|---|
| **エラーも `outputs` に入れる** | 仕様の同期の返しには `outputs` しかない。`error` だけに書いても利用者の画面に届かない |
| **読めなかったら見出しを変える** | 様式を判別できなくても項目の入れ物は出来るので、放っておくと「空欄が並んだそれらしい表」に見える。`gennai.failed()` で見分け、「読み取れませんでした」と返す |
| **1通ずつ順番に読む**（`_gennai_lock`） | 同時に走らせても速くならず、待ち時間が伸びるだけ。下の 9.5.3 も参照 |
| **ファイルは別々の入れ物に置く** | 同じ名前で送られると上書きされ、1ページが黙って消える。名前に連番を足すと結果の「元ファイル」に番号が出てしまうので、フォルダを分ける |
| **HTML の記号を使わない** | `<sub>` が源内の画面で絵になるとは限らない。括弧書きにする |

### 9.5.3 同時に呼ぶとプロセスごと落ちていた（修正済み）

利用者目線の動作確認で見つかりました。読み取り中にもう1通届くと、
**サーバのプロセスが丸ごと終了**し、処理中の全部が巻き添えで消えていました
（3回中2回再現。ログには `GGML_ASSERT ... failed`）。

原因は源内の受け口ではなく**小型LLM**です。`llm.get_assist()` は実体を1つだけ
使い回しますが、サーバは `ThreadingTCPServer` で並行に処理します。
llama.cpp の文脈は同時呼び出しに耐えません。

`LlmAssist` の入り口（`suggest` / `complete`）に錠をかけました。
**源内に限らず、ブラウザ版の利用者も巻き込む不具合でした。**

> 同じ性質のものが他にもあります。VLM は `server._vlm_lock`、
> 出力の抑止は `vlm._SILENCE_LOCK` で守っています。
> **1つの実体を使い回すものを足したら、錠を忘れないこと。**

### 9.5.4 合言葉

```bash
ikensho serve --gennai --gennai-key "決めた合言葉" --host 0.0.0.0
```

`x-api-key` を `hmac.compare_digest` で比べます。日本語の合言葉でも動くよう
**バイト列にしてから**比べます（文字列のままだと `TypeError` になります）。

決めない場合は確かめません。その場合は起動時に警告を出します。

### 9.5.5 版の追随

源内の仕様書は「2026年3月時点のものであり試行錯誤中。今後大きく変わる可能性が
あります」と断っています。実際、選択肢の書き方を `options` / `label` と
取り違えて作り、動作確認で見つかりました（正しくは `items` / `title`）。
**源内側を更新したら `REQUEST_FORM` を仕様書と突き合わせてください。**
...
| `POST /api/gennai` が 404 | 既定で閉じている。`ikensho serve --gennai` で開く（9.5） |
| 源内から呼べない | 既定は `127.0.0.1` にしか出ない。`--host` と、間に置く装置が要る |
| 源内の画面に入力欄が出ない | 「リクエスト形式」の書き方が仕様とずれている。選択肢は `items` / `title`（9.5.5） |
| 同時に呼ぶとサーバが落ちる | 1つの実体を使い回すものに錠が無い（9.5.3）。llama.cpp は同時呼び出しで落ちる |
