...
| **NDLOCR-Lite を入れて比較／文章のように読めるか検討** | ✅ | 72（既定は変えず。行として読む仕組みは様式の見張りに使った） |
| **検証用サンプル2（1,000通）で検証** | ✅ | `seigo2_v1` を追加。枠単位 89.36% / 文字 56.12%。崩れるのは「汚い」（開発メモ 7.7、ISSUE #62） |
| **JSONにこれまでの結果を矛盾なく収める** | ✅ | 読み方を `answers.py`/`answers.js` の1か所に集約。4つの出力すべてがそこを通る |
| **診療科は多いので名前をそのまま出す** | ✅ | 「その他」に書かれた科を名前で置き換える（`option_texts`。9欄14記入欄） |
| **辞書をたくさん作る** | ✅ | 誤字表1,545語・市区町村1,914件。拾っていない30通で 59.46%→59.80%、ブラウザ版 96.7%→97.1% |
| **都道府県・市町村の辞書** | ✅ | 総務省の一覧から1,914件（`tools/fetch_municipalities.py`） |
| **文字起こしが変なのをLLMで自然な日本語に直す** | △ | 規則での訂正は入れた（ブラウザ版も同じ）。**LLM校正は3通りの条件で測って一度も上がらず**、既定では使わない（開発メモ 7.9） |
| **経過と精度をまとめたPPT（文字大きめ）** | ✅ | `docs/主治医意見書読み取り_経過と精度.pptx`（11枚） |
| **Markdownでも出せるように（プロンプトとして読める形）** | ✅ | `--md` / ブラウザ版「Markdownで保存」。Python版とブラウザ版で1行も違わない |
| **テンプレートが少しずつ変わる→全体をOCRしてJSONに組み立てることも検討** | △ | 座標の置き換えにはできなかった（□の検出92.8%・選択肢の言葉69%）。**様式の食い違いを報告する見張り**として実装（開発メモ 7.8、ISSUE #63） |
...
- 期間: 2026-09-06 07:03 〜 2026-09-24 02:10
- コミット 86 件 / 追加 59,396 行 / 削除 5,607 行

### 今のコード量（コメント・空行を含む行数）

| 置き場所 | ファイル | 行 |
|---|---:|---:|
| ブラウザ版（確認・編集画面） | 22 | 8,561 |
| Python版（読み取り本体） | 29 | 7,760 |
| 生成・検証スクリプト | 45 | 8,920 |
...
|---|---:|---:|---:|
| 辞書 | 12,526 | 34 | 14 |
| 様式テンプレート | 9,558 | 769 | 16 |
| ブラウザ版（確認・編集画面） | 9,259 | 591 | 29 |
| 生成・検証スクリプト | 9,240 | 331 | 46 |
| Python版（読み取り本体） | 8,331 | 571 | 30 |
| その他（README・設定など） | 7,442 | 3,172 | 24 |
| 説明書 | 1,349 | 51 | 12 |
| 項目定義 | 1,259 | 82 | 1 |
...
| 72 | 2026-09-12 13:35 | スライドと開発メモを更新し、懸念事項をまとめた | 4 | 526 | 29 |
| 73 | 2026-09-12 13:35 | 作業ログを更新 | 1 | 14 | 4 |
| 74 | 2026-09-20 17:24 | NDLOCR-Lite を組み込んで比べた（既定は変えていない） | 6 | 746 | 0 |
| 75 | 2026-09-22 10:41 | チェック欄の照合は実測で空振りだったので入れない | 2 | 153 | 0 |
| 76 | 2026-09-23 06:09 | 検証用サンプル2の様式を足し、様式の変化を見張るようにした | 22 | 4,443 | 23 |
| 77 | 2026-09-23 06:29 | 測る側の取り違えを直し、枠単位でも数えるようにした | 3 | 160 | 21 |
| 78 | 2026-09-23 06:31 | pagequest の説明文の数字を、後から測り直した値に合わせた | 1 | 4 | 3 |
| 79 | 2026-09-23 06:32 | 開発者向けに seigo2 での測り方を足した | 1 | 32 | 0 |
| 80 | 2026-09-23 07:37 | 64通で測り直した。崩れているのは「汚い」に集中していた | 4 | 80 | 18 |
| 81 | 2026-09-23 07:37 | 作業ログを更新 | 1 | 95 | 13 |
| 82 | 2026-09-23 08:22 | 出力の答えの読み方を1か所にまとめ、Markdown でも出せるようにした | 15 | 826 | 19 |
| 83 | 2026-09-23 08:23 | 作業ログを更新 | 1 | 40 | 12 |
| 84 | 2026-09-23 16:19 | 文字起こしの崩れを規則で直す。LLM校正は測った結果、既定では使わない | 12 | 841 | 38 |
| 85 | 2026-09-23 16:19 | 作業ログを更新 | 1 | 37 | 10 |
| 86 | 2026-09-24 02:10 | 誤字表を1,545語に増やし、市区町村の辞書を足した | 16 | 11,221 | 13 |
...
### 72. スライドと開発メモを更新し、懸念事項をまとめた

`52b0010` 2026-09-12 13:35 — 4 ファイル ／ +526 −29

- `WORKLOG.md` +257 −11
- `tools/build_pptx.py` +127 −18
- `DEVNOTES.md` +142 −0
- `"docs/\344\270\273\346\262\273\345\214\273\346\204\217\350\246\213\346\233\270\350\252\255\343\201\277\345\217\226\343\202\212_\346\246\202\350\246\201.pptx"` +0 −0

### 73. 作業ログを更新

`744c821` 2026-09-12 13:35 — 1 ファイル ／ +14 −4

- `WORKLOG.md` +14 −4

### 74. NDLOCR-Lite を組み込んで比べた（既定は変えていない）

`620b749` 2026-09-20 17:24 — 6 ファイル ／ +746 −0

- `python/ikensho_ocr/ndlocr.py` +278 −0
- `tools/benchmark_ndlocr.py` +215 −0
- `tools/fetch_ndlocr.py` +115 −0
- `DEVNOTES.md` +94 −0
- `README.md` +33 −0
- `python/ikensho_ocr/ocr.py` +11 −0

### 75. チェック欄の照合は実測で空振りだったので入れない

`c982405` 2026-09-22 10:41 — 2 ファイル ／ +153 −0

- `tools/check_ndl_marks.py` +97 −0
- `python/ikensho_ocr/ndlocr.py` +56 −0

### 76. 検証用サンプル2の様式を足し、様式の変化を見張るようにした

`9abaa0e` 2026-09-23 06:09 — 22 ファイル ／ +4,443 −23

- `templates/seigo2_v1.json` +2864 −0
- `tools/build_template_from_source.py` +321 −0
- `tools/benchmark_seigo2.py` +277 −0
- `python/ikensho_ocr/boxfind.py` +189 −0
- `python/ikensho_ocr/pagequest.py` +187 −0
- `tools/benchmark_labels.py` +179 −0
- `DEVNOTES.md` +161 −0
- `python/ikensho_ocr/labels.py` +105 −10
- `python/ikensho_ocr/extract.py` +56 −10
- `tools/dump_layout.js` +41 −0
- `web/js/labels.js` +16 −3
- `README.md` +10 −0
- `python/ikensho_ocr/export.py` +9 −0
- `python/ikensho_ocr/ocr.py` +9 −0
- `python/ikensho_ocr/ndlocr.py` +7 −0
- ほか 7 ファイル +12 −0

### 77. 測る側の取り違えを直し、枠単位でも数えるようにした

`0a12ad3` 2026-09-23 06:29 — 3 ファイル ／ +160 −21

- `tools/benchmark_seigo2.py` +108 −6
- `DEVNOTES.md` +44 −9
- `README.md` +8 −6

### 78. pagequest の説明文の数字を、後から測り直した値に合わせた

`a762caa` 2026-09-23 06:31 — 1 ファイル ／ +4 −3

- `python/ikensho_ocr/pagequest.py` +4 −3

### 79. 開発者向けに seigo2 での測り方を足した

`4b09b8b` 2026-09-23 06:32 — 1 ファイル ／ +32 −0

- `docs/DEVELOPER.md` +32 −0

### 80. 64通で測り直した。崩れているのは「汚い」に集中していた

`e7cad8f` 2026-09-23 07:37 — 4 ファイル ／ +80 −18

- `DEVNOTES.md` +36 −11
- `tools/build_pptx.py` +27 −3
- `README.md` +17 −4
- `"docs/\344\270\273\346\262\273\345\214\273\346\204\217\350\246\213\346\233\270\350\252\255\343\201\277\345\217\226\343\202\212_\346\246\202\350\246\201.pptx"` +0 −0

### 81. 作業ログを更新

`de18a68` 2026-09-23 07:37 — 1 ファイル ／ +95 −13

- `WORKLOG.md` +95 −13

### 82. 出力の答えの読み方を1か所にまとめ、Markdown でも出せるようにした

`7afa9a0` 2026-09-23 08:22 — 15 ファイル ／ +826 −19

- `python/ikensho_ocr/export.py` +195 −5
- `web/js/exporters.js` +153 −6
- `python/ikensho_ocr/answers.py` +112 −0
- `web/js/answers.js` +99 −0
- `DEVNOTES.md` +62 −1
- `tools/build_schema.py` +59 −0
- `docs/DEVELOPER.md` +39 −3
- `README.md` +37 −0
- `schema/ikensho.schema.json` +32 −0
- `tools/form_definition.py` +11 −0
- `python/ikensho_ocr/cli.py` +8 −2
- `web/index.html` +6 −1
- `web/js/app.js` +5 −1
- `python/ikensho_ocr/schema.py` +5 −0
- `WORKLOG.md` +3 −0

### 83. 作業ログを更新

`e955608` 2026-09-23 08:23 — 1 ファイル ／ +40 −12

- `WORKLOG.md` +40 −12

### 84. 文字起こしの崩れを規則で直す。LLM校正は測った結果、既定では使わない

`ee477dd` 2026-09-23 16:19 — 12 ファイル ／ +841 −38

- `tools/build_pptx_summary.py` +286 −0
- `python/ikensho_ocr/proofread.py` +165 −18
- `tools/benchmark_proofread.py` +117 −0
- `DEVNOTES.md` +106 −0
- `python/ikensho_ocr/extract.py` +63 −4
- `web/js/dicts.js` +46 −0
- `README.md` +24 −4
- `python/ikensho_ocr/llm.py` +11 −4
- `web/js/exporters.js` +9 −4
- `python/ikensho_ocr/export.py` +8 −3
- `python/ikensho_ocr/cli.py` +6 −1
- `"docs/\344\270\273\346\262\273\345\214\273\346\204\217\350\246\213\346\233\270\350\252\255\343\201\277\345\217\226\343\202\212_\347\265\214\351\201\216\343\201\250\347\262\276\345\272\246.pptx"` +0 −0

### 85. 作業ログを更新

`191139f` 2026-09-23 16:19 — 1 ファイル ／ +37 −10

- `WORKLOG.md` +37 −10

### 86. 誤字表を1,545語に増やし、市区町村の辞書を足した

`68b103d` 2026-09-24 02:10 — 16 ファイル ／ +11,221 −13

- `dict/municipalities.json` +7664 −0
- `dict/word_fixes.json` +1473 −0
- `dict/ocr_confusions.json` +828 −0
- `dict/care_terms.json` +384 −0
- `tools/mine_word_fixes.py` +255 −0
- `tools/build_word_fixes.py` +154 −0
- `dict/mined_fixes.json` +136 −0
- `tools/fetch_municipalities.py` +129 −0
- `DEVNOTES.md` +59 −0
- `docs/DEVELOPER.md` +49 −0
- `python/ikensho_ocr/proofread.py` +29 −1
- `python/ikensho_ocr/dictionaries.py` +18 −5
- `web/js/dicts.js` +20 −3
- `README.md` +20 −2
- `dict/field_map.json` +2 −2
- ほか 1 ファイル +1 −0
